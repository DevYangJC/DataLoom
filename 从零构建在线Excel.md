...
**前端是一个独立的 SPA 应用。** 它不依赖特定的后端框架，跟后端只通过 REST API 通信。你后端是 Spring Boot、Go、Node，甚至 Python Flask，都无所谓——把 `dataloom-web/src` 目录往你项目里一扔，配个 API 代理就行。前端不认后端是谁，只认 `/api/excel` 这个前缀。
...
协同编辑是整个项目里最难啃的骨头。核心思路是给 `dataloom-server` 扩展一个独立的 `socket-service`，新增 `ExcelWebSocketServer`，让前端通过 WebSocket 把每次编辑动作实时广播出去。
